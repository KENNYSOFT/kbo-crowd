...
그 다음 `dashboard` 디렉토리를 정적 서버로 열면 된다. 맨 위 두 절은 묻는 것이 다르다. **예매 난이도**는 내일부터 일주일간 열릴 경기를 날짜별로 놓고 매진 확률을 보여준다. 아직 표를 살 수 있는 경기이고, 예매가 대체로 경기 일주일쯤 전에 열리므로 다음 경기 하나가 아니라 그 창 전체를 본다. **오늘 관중**은 오늘 경기의 예상 관중 수를 보여준다. 오늘은 표가 이미 팔렸거나 당일권만 남아서 물을 것이 표를 구할 수 있느냐가 아니라 몇 명이 오느냐다. 한 번의 학습으로 두 갈래를 모두 내므로 두 절의 숫자가 서로 어긋날 수 없다. 그 아래로는 구장 정렬 기준을 화면에서 고를 수 있고, 무엇으로 정렬했는지 차트 위에 표시된다. 수용인원 표도 같은 순서를 따라가므로 두 화면을 나란히 대조할 수 있다.
