...
그 다음 `dashboard` 디렉토리를 정적 서버로 열면 된다. 맨 위에 앞으로 일주일간 열릴 경기의 티켓 난이도가 날짜별로 온다. 예매가 대체로 경기 일주일쯤 전에 열리므로 다음 경기 하나가 아니라 그 창 전체를 본다. 그 아래로는 구장 정렬 기준을 화면에서 고를 수 있고, 무엇으로 정렬했는지 차트 위에 표시된다. 수용인원 표도 같은 순서를 따라가므로 두 화면을 나란히 대조할 수 있다.
