...
비가 어느 단계에서 작동하는지를 주중과 주말로 가를 수 있다. 주말은 표가 일찍 팔리고 주중은 당일 판매 비중이 크다. 수요가 빠듯한 경기만 놓고 보면 비 오는 날 매진율이 **주중은 38.0%p 떨어지는데 주말은 8.4%p 밖에 안 떨어진다**. 지붕이 있는 고척에서는 아예 떨어지지 않는다. 즉 비는 표를 미리 사는 단계가 아니라 당일 단계에서 작동한다. 일주일 전에 팔리는 표는 그날 날씨를 보고 팔리는 것이 아니다.

그래서 예보가 100% 맞는다고 가정해도 매진 예측은 나아지지 않는다. 오히려 나빠진다.

| | 브라이어 | 로그손실 | AUC |
|---|---|---|---|
| 날씨 없음 | 0.1595 | 0.4944 | 0.934 |
| 날씨 있음 (완벽한 예보 가정) | 0.1629 | 0.4978 | 0.932 |
| 기준선 (평균으로 찍기) | 0.2312 | 0.6552 | - |

완벽한 예보로도 이득이 없으니 부정확한 중기예보는 볼 것도 없다. 야외 경기 2,435건 중 경기 시간대에 비가 온 것이 116건(4.8%)뿐이라, 날씨 변수 여덟 개가 배우는 것보다 끌고 오는 노이즈가 크다.

관중 수 예측은 다를 것 같다. 매진은 이미 팔린 표가 지켜 주지만 관중 수는 당일 판매가 줄면 그대로 깎이기 때문이다. 그런데 비 온 경기의 오차를 여섯 구간으로 재 보면 2025년 구간에서는 날씨를 넣어 12%와 32% 나빠지고 2026년 후반에서는 14%에서 45%까지 좋아진다. 가중 평균은 -1.1% 로 차이가 없다. 구간마다 부호가 뒤집히는 것은 효과가 아니라 비 온 경기가 구간당 열댓 건뿐이라는 뜻이다. 한 구간만 재고 "비 오는 날엔 날씨가 듣는다" 고 결론내지 않도록 `lead_time.py` 가 여섯 구간을 함께 찍는다.

그럼 티켓이 열리는 시점에 예보하는 것 자체는 얼마나 손해일까. 순위와 연승을 7일 전 값으로 되돌려 보면 브라이어가 0.1595 에서 0.1609 로 오르는 데 그치고 AUC 는 0.934 에서 0.933 으로 사실상 같다. 순위가 그 사이 평균 0.7계단밖에 움직이지 않기 때문이다. 연승은 2.9경기 움직여 변동이 큰데, 그 값을 평년으로 당겨 믿음을 낮춰 봐도 0.1603 으로 손댈 만한 차이가 아니다. **일주일 전 예보가 당일 예보와 거의 같은 품질이라는 뜻이고**, 이 대시보드가 예매창이 열리는 날짜를 미리 보여줄 수 있는 근거다.
