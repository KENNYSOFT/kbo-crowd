...
| `data/holidays.csv` | 날짜 | 공휴일, 대체공휴일, 임시공휴일, 선거일 (손으로 관리) |
...
공휴일도 원자료에 없어서 `data/holidays.csv` 에 손으로 넣는다. 대체공휴일과 임시공휴일, 선거일이 섞여 있어 규칙으로 만들 수 없다. 다음 해 월력요항은 우주항공청이 전해 6월쯤 관보에 내므로 그것을 옮기고, 임시공휴일과 선거일은 발표되면 더한다. 목록에 없는 해의 경기를 예측하려 하면 모델이 멈춘다. 평일로 보고 넘어가면 그해 공휴일 경기가 전부 조용히 틀리기 때문이다. 넣은 뒤에는 일정과 맞춰 볼 수 있다. KBO 는 평일 공휴일에 저녁 6시 반보다 이른 경기를 편성하므로, 그런 경기가 있는데(더블헤더 제외) 목록에 없는 평일은 빠뜨린 것이다. 모델은 이 가운데 평일 공휴일만 쓴다. 주말 공휴일은 요일만으로 이미 맞게 보고, 거기에 더하면 주말을 두 번 센다.
...
비가 어느 단계에서 작동하는지를 주중과 주말로 가를 수 있다. 주말은 표가 일찍 팔리고 주중은 당일 판매 비중이 크다. 수요가 빠듯한 경기만 놓고 보면 비 오는 날 매진율이 **주중은 41.3%p 떨어지는데 주말은 8.4%p 밖에 안 떨어진다**. 지붕이 있는 고척에서는 아예 떨어지지 않는다. 즉 비는 표를 미리 사는 단계가 아니라 당일 단계에서 작동한다. 일주일 전에 팔리는 표는 그날 날씨를 보고 팔리는 것이 아니다.

그래서 예보가 100% 맞는다고 가정해도 매진 예측은 거의 나아지지 않는다.

| | 브라이어 | 로그손실 | AUC |
|---|---|---|---|
| 날씨 없음 | 0.1491 | 0.4621 | 0.931 |
| 날씨 있음 (완벽한 예보 가정) | 0.1485 | 0.4537 | 0.933 |
| 기준선 (평균으로 찍기) | 0.2313 | 0.6554 | - |

완벽한 예보로도 브라이어가 0.0006 줄 뿐이니 부정확한 중기예보는 볼 것도 없다. 야외 경기 2,445건 중 경기 시간대에 비가 온 것이 116건(4.7%)뿐이라, 날씨 변수 여덟 개가 배울 것이 거의 없다.

관중 수 예측은 다를 것 같다. 매진은 이미 팔린 표가 지켜 주지만 관중 수는 당일 판매가 줄면 그대로 깎이기 때문이다. 그런데 비 온 경기의 오차를 여섯 구간으로 재 보면 2025년 두 구간과 2026년 5월 구간에서는 날씨를 넣어 6%에서 35% 나빠지고, 2026년 6월 이후 세 구간에서는 13%에서 34%까지 좋아진다. 가중 평균은 -3.0% 로 오히려 조금 나빠진다. 구간마다 부호가 뒤집히는 것은 효과가 아니라 비 온 경기가 구간당 열댓 건뿐이라는 뜻이다. 한 구간만 재고 "비 오는 날엔 날씨가 듣는다" 고 결론내지 않도록 `lead_time.py` 가 여섯 구간을 함께 찍는다.

그럼 티켓이 열리는 시점에 예보하는 것 자체는 얼마나 손해일까. 순위와 연승을 7일 전 값으로 되돌려 보면 브라이어가 0.1491 에서 0.1513 으로 오르는 데 그치고 AUC 는 0.931 에서 0.930 으로 사실상 같다. 순위가 그 사이 평균 0.7계단밖에 움직이지 않기 때문이다. 연승은 평균 3.0경기 움직여 변동이 크지만, 그 변동까지 들어간 결과가 이 정도다. **일주일 전 예보가 당일 예보와 거의 같은 품질이라는 뜻이고**, 이 대시보드가 예매창이 열리는 날짜를 미리 보여줄 수 있는 근거다.
...
`analysis/test_model.py` 를 먼저 돌린다. 참값을 아는 합성 데이터로 검열 회귀가 모수를 되찾는지, 기울기가 수치 미분과 맞는지, 검열이 없을 때 최소제곱과 같아지는지를 본다. 시즌 곡선이 월이 바뀌는 날에도 계단 없이 이어지는지, 평일 공휴일을 가르고 목록에 없는 해에서 멈추는지도 함께 본다. 실제 관중 데이터로는 맞았는지 확인할 방법이 없다. 매진 경기의 진짜 수요는 아무도 모르기 때문이다.
